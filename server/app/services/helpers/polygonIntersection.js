...
function verifactions(fig1, fig2) {
...
//check the polygons are 'near'
function checkProximity(fig1, fig2) {
    return findPolar(true, false, fig1) > findPolar(true, true, fig2) &&
        findPolar(true, true, fig1) < findPolar(true, false, fig2) &&
        findPolar(false, false, fig1) > findPolar(false, true, fig2) &&
        findPolar(false, true, fig1) < findPolar(false, false, fig2)
}

function findPolar(isX, isMin, fig) {
    const coord = isX ? "x" : "y";
    let value = fig[0][coord];
    fig.forEach(pos => {
        if (isMin) {
...
function classifyPoint(p, edge) {
    ...
    if (typeof theta==="undefined") {return false}
    if (theta < 0) {
        theta = theta + 360;
    }
    if (sa < -0.0000000001) {
...
module.exports = {
    intersect: intersect,
    calcPolygonArea: calcPolygonArea,
